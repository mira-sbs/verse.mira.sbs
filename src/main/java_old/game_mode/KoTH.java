...
import sbs.mira.verse.util.WoolColor;
import org.bukkit.*;
...
public class KoTH extends Gamemode {
    ...
    private void doFireworks() {
        if (holder == null) // Spawn white.
            (( MiraVerseDataModel ) main).entities( ).spawn_firework( flag.clone( ).add( 0.5, 1, 0.5 ), ChatColor.WHITE );
        else
            (( MiraVerseDataModel ) main).entities( ).spawn_firework( flag.clone( ).add( 0.5, 1, 0.5 ), holder.getTeamColor( ) );
    }
