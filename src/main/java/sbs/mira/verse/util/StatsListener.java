...
import sbs.mira.verse.framework.event.MatchEndEvent;
import sbs.mira.verse.framework.event.MatchPlayerDeathEvent;
...
public class StatsListener extends MiraModule implements Listener {
    ...
    public void onServerList(ServerListPingEvent event) {
        MiraMapOld_DELETEME map = ( MiraMapOld_DELETEME ) mira( ).cache( ).getCurrentMap( );
        String state;
    ...
    public void onPreLogin(AsyncPlayerPreLoginEvent event) {
        ...
        try {
            ...
            PreparedStatement stats = (( MiraVerseDataModel ) mira( )).db( ).prepare( "SELECT * FROM `WarStats` WHERE `player_uuid`=?" );
            ...
            if (check.next()) {
                ...
                (( MiraVerseDataModel ) mira( )).putTempStats( event.getUniqueId( ), new WarStats( ( MiraVerseDataModel ) mira( ), event.getUniqueId( ),
                ...
                PreparedStatement newStats = (( MiraVerseDataModel ) mira( )).db( ).prepare( "INSERT INTO `WarStats` (`player_uuid`) VALUES (?)" );
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        if (status == WarMatch.Status.STARTING || status == WarMatch.Status.PLAYING || status == WarMatch.Status.CYCLE)
            ...
            target.teleport((( MiraMapOld_DELETEME ) mira( ).cache( ).getMap( mira( ).match( ).getPreviousMap( ) )).getSpectatorSpawn_( ) ); // Spawn them in the previous defined map.
    ...
    private void awardVote(Player target) {
        ...
        (( MiraVerseDataModel ) mira( )).entities( ).spawn_firework( target.getLocation( ) );
